...
use Generated\Interfaces\CommentRepositoryInterface;
use App\Config\Database;
use PDO;

class Comment implements CommentRepositoryInterface
{
    private $db;
    ...
    private function loadRelations(array $item): array
    {
        ...
        // Relación: comments.post_id -> posts
        if (isset($item['post_id']) && $item['post_id']) {
            $stmt = $this->db->prepare("SELECT * FROM `posts` WHERE id = ?");
            $stmt->execute([$item['post_id']]);
            $item['post_data'] = $stmt->fetch(PDO::FETCH_ASSOC);
        }
